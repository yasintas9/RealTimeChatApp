...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == view_send) {
            view= LayoutInflater.from(context).inflate(R.layout.sendlayout,parent,false);
            return new ViewHolder(view);
        }else{
            view= LayoutInflater.from(context).inflate(R.layout.receiverlayout,parent,false);
            return new ViewHolder(view);
        }
    ...
    public int getItemViewType(int position) {
        if(list.get(position).getFrom().equals(username)){ // gönderen kişi username ise
                state = true;
                return view_send;
        }else{
            state = false;
